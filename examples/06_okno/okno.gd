...
## Трек «Пока горит окно» с лентой событий: слышно и видно разом.
##
...
const HERE := "res://examples/06_okno"
const TRACK_FILE := HERE + "/window_light.js"
...
func _ready() -> void:
	...
	_music.samples_path = HERE          # рояль лежит рядом, 42 записи
	add_child(_music)
	_music.event_played.connect(_on_event)
	var f := FileAccess.open(TRACK_FILE, FileAccess.READ)
	if f == null:
		push_error("нет трека: " + TRACK_FILE)
		return
	_music.play(f.get_as_text())
	f.close()
	var bank := _music.get_bank()
	print("Банк: %d наборов. «Пока горит окно» — 72 цикла, ~3:25. Esc — выход." % (bank.count() if bank else 0))
