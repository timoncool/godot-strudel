...
func _statement() -> Dictionary:
	...
	if _is_p("$") and _is_p(":", 1):
		label = "$"
		_i += 2
	# 🔴 СКОБКА ПОСЛЕ МЕТКИ — ЭТО ВСЁ ЕЩЁ МЕТКА. Оговорка `not _is_p("(", 2)`
	# отвергала строку вида `$: (…)`, и она падала разбором: «не ожидал здесь
	# ":"». На уровне ПРЕДЛОЖЕНИЯ пара «имя, двоеточие» иначе как меткой быть
	# не может — ключ объекта разбирается в другом месте.
	elif _peek().get("t", "") == "id" and _is_p(":", 1):
