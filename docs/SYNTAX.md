...
0. **Общий генератор игры не трогается.** Шум и стая пил берут числа из
   СВОЕГО `RandomNumberGenerator`. Иначе каждая сыгранная нота сдвигала бы
   череду `randf()`, а ею игра раскладывает свои вещи — и звук молча менял
   бы ход игры. Зерно шума задаётся `StrudelVoice.set_random_seed()`.
